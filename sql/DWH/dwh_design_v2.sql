CREATE SCHEMA IF NOT EXISTS dwh_v2;
...
CREATE TABLE IF NOT EXISTS dwh_v2.dim_date (
...
CREATE TABLE IF NOT EXISTS dwh_v2.dim_country (
...
CREATE TABLE IF NOT EXISTS dwh_v2.dim_city (
    city_key bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    source_city_id integer NOT NULL UNIQUE,
    state_province_key bigint REFERENCES dwh_v2.dim_state_province(state_province_key),
    city_name varchar(50) NOT NULL,
    latest_recorded_population bigint
);

CREATE TABLE IF NOT EXISTS dwh_v2.dim_customer_category (
...
CREATE TABLE IF NOT EXISTS dwh_v2.dim_buying_group (
...
CREATE TABLE IF NOT EXISTS dwh_v2.dim_delivery_method (
...
CREATE TABLE IF NOT EXISTS dwh_v2.dim_payment_method (
...
CREATE TABLE IF NOT EXISTS dwh_v2.dim_transaction_type (
...
CREATE TABLE IF NOT EXISTS dwh_v2.dim_package_type (
...
CREATE TABLE IF NOT EXISTS dwh_v2.dim_stock_group (
...
CREATE TABLE IF NOT EXISTS dwh_v2.dim_person (
...
CREATE TABLE IF NOT EXISTS dwh_v2.dim_supplier (
    ...
    delivery_city_key bigint REFERENCES dwh_v2.dim_city(city_key),
    postal_city_key bigint REFERENCES dwh_v2.dim_city(city_key),
    delivery_method_key bigint REFERENCES dwh_v2.dim_delivery_method(delivery_method_key),
    payment_days integer,
    phone_number varchar(20),
    website_url varchar(256)
);

CREATE TABLE IF NOT EXISTS dwh_v2.dim_customer (
    customer_key bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    source_customer_id integer NOT NULL UNIQUE,
    customer_name varchar(100) NOT NULL,
    customer_category_key bigint REFERENCES dwh_v2.dim_customer_category(customer_category_key),
    buying_group_key bigint REFERENCES dwh_v2.dim_buying_group(buying_group_key),
    delivery_method_key bigint REFERENCES dwh_v2.dim_delivery_method(delivery_method_key),
    delivery_city_key bigint REFERENCES dwh_v2.dim_city(city_key),
    postal_city_key bigint REFERENCES dwh_v2.dim_city(city_key),
    ...
    phone_number varchar(20),
    website_url varchar(256),
    base_credit_limit numeric(18,2),
    base_payment_days integer
);

CREATE TABLE IF NOT EXISTS dwh_v2.dim_product (
    product_key bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    source_stock_item_id integer NOT NULL UNIQUE,
    stock_item_name varchar(100) NOT NULL,
    supplier_key bigint REFERENCES dwh_v2.dim_supplier(supplier_key),
    color_name varchar(20),
    unit_package_type_key bigint REFERENCES dwh_v2.dim_package_type(package_type_key),
    outer_package_type_key bigint REFERENCES dwh_v2.dim_package_type(package_type_key),
    brand varchar(50),
    size varchar(20),
...
CREATE TABLE IF NOT EXISTS dwh_v2.bridge_product_stock_group (
    product_key bigint NOT NULL REFERENCES dwh_v2.dim_product(product_key),
    stock_group_key bigint NOT NULL REFERENCES dwh_v2.dim_stock_group(stock_group_key),
    PRIMARY KEY (product_key, stock_group_key)
);

-- Base measures cho Sales Invoice Line
CREATE TABLE IF NOT EXISTS dwh_v2.dim_sales_line_base (
    sales_line_base_key bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    source_invoice_line_id integer NOT NULL UNIQUE,
    product_key bigint NOT NULL REFERENCES dwh_v2.dim_product(product_key),
    
    quantity_sold numeric(18, 3) NOT NULL,
    unit_price_ex_tax numeric(18, 2) NOT NULL,
    extended_price_ex_tax numeric(18, 2) NOT NULL,
    tax_amount numeric(18, 2) NOT NULL,
    line_profit_base numeric(18, 2)          -- từ staging.line_profit
);

-- Base measures cho Order Fulfillment Line  
CREATE TABLE IF NOT EXISTS dwh_v2.dim_order_line_base (
    order_line_base_key bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS dwh_v2.dim_customer_transaction_base (
    customer_transaction_base_key bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    source_customer_transaction_id integer NOT NULL UNIQUE,
    
    -- === BASE MEASURES lấy trực tiếp từ staging ===
    ...
    transaction_type_key bigint REFERENCES dwh_v2.dim_transaction_type(transaction_type_key),
    payment_method_key bigint REFERENCES dwh_v2.dim_payment_method(payment_method_key)
);


CREATE TABLE IF NOT EXISTS dwh_v2.fact_sales_invoice_line (
    sales_invoice_line_key bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    source_invoice_line_id integer NOT NULL UNIQUE,
    source_invoice_id integer NOT NULL,
    invoice_date_key integer NOT NULL REFERENCES dwh_v2.dim_date(date_key),
    customer_key bigint NOT NULL REFERENCES dwh_v2.dim_customer(customer_key),
    bill_to_customer_key bigint REFERENCES dwh_v2.dim_customer(customer_key),
    product_key bigint NOT NULL REFERENCES dwh_v2.dim_product(product_key),
    package_type_key bigint REFERENCES dwh_v2.dim_package_type(package_type_key),
    salesperson_key bigint REFERENCES dwh_v2.dim_person(person_key),
    sales_line_base_key bigint NOT NULL REFERENCES dwh_v2.dim_sales_line_base(sales_line_base_key),

    -- === CHỈ DERIVED MEASURES ===
...
CREATE TABLE IF NOT EXISTS dwh_v2.fact_order_fulfillment_line (
    order_fulfillment_line_key bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    source_order_line_id integer NOT NULL UNIQUE,
    source_order_id integer NOT NULL,
    order_date_key integer NOT NULL REFERENCES dwh_v2.dim_date(date_key),
    expected_delivery_date_key integer REFERENCES dwh_v2.dim_date(date_key),
    picking_completed_date_key integer REFERENCES dwh_v2.dim_date(date_key),
    customer_key bigint NOT NULL REFERENCES dwh_v2.dim_customer(customer_key),
    product_key bigint NOT NULL REFERENCES dwh_v2.dim_product(product_key),
    package_type_key bigint REFERENCES dwh_v2.dim_package_type(package_type_key),
    salesperson_key bigint REFERENCES dwh_v2.dim_person(person_key),
    picker_key bigint REFERENCES dwh_v2.dim_person(person_key),
    order_line_base_key bigint NOT NULL REFERENCES dwh_v2.dim_order_line_base(order_line_base_key),
...
CREATE TABLE IF NOT EXISTS dwh_v2.fact_customer_transaction (
    customer_transaction_key bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    source_customer_transaction_id integer NOT NULL UNIQUE,
    source_invoice_id integer,
    transaction_date_key integer NOT NULL REFERENCES dwh_v2.dim_date(date_key),
    finalization_date_key integer REFERENCES dwh_v2.dim_date(date_key),
    due_date_key integer REFERENCES dwh_v2.dim_date(date_key),
    customer_key bigint NOT NULL REFERENCES dwh_v2.dim_customer(customer_key),
    
    -- Link đến base dimension
    customer_transaction_base_key bigint NOT NULL 
        REFERENCES dwh_v2.dim_customer_transaction_base(customer_transaction_base_key),
...
CREATE INDEX IF NOT EXISTS idx_fact_sales_invoice_line_date
    ON dwh_v2.fact_sales_invoice_line(invoice_date_key);
CREATE INDEX IF NOT EXISTS idx_fact_sales_invoice_line_customer
    ON dwh_v2.fact_sales_invoice_line(customer_key);
CREATE INDEX IF NOT EXISTS idx_fact_sales_invoice_line_product
    ON dwh_v2.fact_sales_invoice_line(product_key);

CREATE INDEX IF NOT EXISTS idx_fact_order_fulfillment_line_date
    ON dwh_v2.fact_order_fulfillment_line(order_date_key);
CREATE INDEX IF NOT EXISTS idx_fact_order_fulfillment_line_customer
    ON dwh_v2.fact_order_fulfillment_line(customer_key);
CREATE INDEX IF NOT EXISTS idx_fact_order_fulfillment_line_product
    ON dwh_v2.fact_order_fulfillment_line(product_key);

CREATE INDEX IF NOT EXISTS idx_fact_customer_transaction_date
    ON dwh_v2.fact_customer_transaction(transaction_date_key);
CREATE INDEX IF NOT EXISTS idx_fact_customer_transaction_customer
    ON dwh_v2.fact_customer_transaction(customer_key);
CREATE INDEX IF NOT EXISTS idx_fact_customer_transaction_due_date
    ON dwh_v2.fact_customer_transaction(due_date_key);

CREATE TABLE IF NOT EXISTS dwh_v2.fact_business_kpi_month (
    period_date_key integer PRIMARY KEY REFERENCES dwh_v2.dim_date(date_key),
...
CREATE TABLE IF NOT EXISTS dwh_v2.fact_customer_kpi_month (
    customer_key bigint NOT NULL REFERENCES dwh_v2.dim_customer(customer_key),
    period_date_key integer NOT NULL REFERENCES dwh_v2.dim_date(date_key),
...
CREATE INDEX IF NOT EXISTS idx_fact_customer_kpi_month_period
    ON dwh_v2.fact_customer_kpi_month(period_date_key);
